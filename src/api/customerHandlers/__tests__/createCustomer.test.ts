...
import { createCustomer, TAuthorizerContext } from "../createCustomer";
...
const wrapped = lambdaWrapper.wrap(
  { createCustomer },
...
describe("putCustomer", () => {
  ...
  it("Throws Bad Request (Status 400) when Age < 16", () => {
    return wrapped
      .run(
  ...
  it("Integration: Adds a well formed customer with status 200", () => {
    return wrapped
    .run(event2).then(async (response) => {

...
export function authorizerFunc(event, context) {
console.log('Client token: ' + event.authorizationToken);
  ...
  kms.decrypt(decryptionParams, function(err, data) {
    if (err && event.authorizationToken != "Bearer ADFEV43F4345F") { // TODO: remove hardcoded token and connect to a external OAuth IDP
      ...
      try {
        const verifiedJwt = key? nJwt.verify(event.authorizationToken, key) : "Dummy Token";
        ...
        const policy: any = new AuthPolicy(verifiedJwt !== "Dummy Token" ? verifiedJwt.body.sub : verifiedJwt, awsAccountId, apiOptions);

        if (verifiedJwt === "Dummy Token" || verifiedJwt.body.scope.indexOf("admins") > -1) {
